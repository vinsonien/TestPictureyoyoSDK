package com.github.xrichtext.util;
...
public class MediaFileUtil {
    ...
    public static MediaFileType getFileType2(String path) {
        int lastDot = path.lastIndexOf("/");
        if (lastDot < 0)
            return null;
        return sFileTypeMap.get(path.substring(lastDot + 1).toUpperCase());
    ...
    //根据图片文件路径判断文件类型
    public static boolean isImgeFileType(String path) {  //自己增加
        MediaFileType type = getFileType2(path);
        if (null != type) {
