...
//    private LinearLayout allLayout; // 这个是所有子view的容器，scrollView内部的唯一一个ViewGroup
    ...
    public LayoutTransition mTransitioner; // 只在图片View添加或remove时，触发transition动画
    public int editNormalPadding = 0; //
    public int disappearingImageIndex = 0;
    //private Bitmap bmp;
    ...
    //删除图片的接口
    public OnRtImageDeleteListener onRtImageDeleteListener;
    ...
    public RichTextEditor(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        //获取自定义属性
        TypedArray ta = context.obtainStyledAttributes(attrs, R.styleable.RichScrollTextEditor);
        rtImageHeight = ta.getInteger(R.styleable.RichScrollTextEditor_rt_editor_image_height, 500);
        rtImageBottom = ta.getInteger(R.styleable.RichScrollTextEditor_rt_editor_image_bottom, 10);
        rtTextSize = ta.getDimensionPixelSize(R.styleable.RichScrollTextEditor_rt_editor_text_size, 16);
        //rtTextSize = ta.getInteger(R.styleable.RichTextView_rt_view_text_size, 16);
        rtTextLineSpace = ta.getDimensionPixelSize(R.styleable.RichScrollTextEditor_rt_editor_text_line_space, 8);
        rtTextColor = ta.getColor(R.styleable.RichScrollTextEditor_rt_editor_text_color, Color.parseColor("#757575"));
        rtTextInitHint = ta.getString(R.styleable.RichScrollTextEditor_rt_editor_text_init_hint);
...
//				.start(activity);
    ...
    public void insertImage(Bitmap bitmap, String imagePath) {
        ...
        int lastEditIndex = indexOfChild(lastFocusEdit);//获取焦点的EditText所在位置
    ...
    public void addEditTextAtIndex(final int index, CharSequence editStr) {
        ...
        EditText editText2 = createEditText(rtTextInitHint, EDIT_PADDING);
    ...
    public void addImageViewAtIndex(final int index, Bitmap bmp, String imagePath) {
        ...
        if (rtImageHeight > 0) {
            ...
            imageHeight = getWidth() * bmp.getHeight() / bmp.getWidth();
        }
